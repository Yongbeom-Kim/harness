...
const defaultSessionName = "codex"
...
func runCodex(cfg runnerConfig, parsed parsedArgs) int {
	...
	if err := pane.SendText(cfg.buildLaunch("codex")); err != nil {
		_ = session.Close()
		fmt.Fprintln(cfg.stderr, err.Error())
		return 1
	}
	...
	fmt.Fprintf(cfg.stdout, "Launched Codex in tmux session %q\n", session.AttachTarget())
	return 0
}
