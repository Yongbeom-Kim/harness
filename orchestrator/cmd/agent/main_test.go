...
func TestRunAgentLaunchesWithoutAttach(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	pane := &fakePane{target: "agent:0.0"}
	session := &fakeSession{name: "agent", pane: pane}

	exitCode := runAgent(NewRunnerConfig(
		WithStdout(&stdout),
		WithStderr(&stderr),
	...
	if len(pane.sent) != 1 || pane.sent[0] != "launch agent" {
		t.Fatalf("unexpected sent launcher: %v", pane.sent)
	}
	if !strings.Contains(stdout.String(), `Launched Agent in tmux session "agent"`) {
...
func TestRunAgentAttachesWhenRequested(t *testing.T) {
	stdin := strings.NewReader("interactive input")
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	pane := &fakePane{}
	session := &fakeSession{name: "agent-dev", pane: pane}
	...
	exitCode := runAgent(NewRunnerConfig(
		...
		WithBuildLaunch(func(string, ...string) string { return "launch agent" }),
	), parsedArgs{sessionName: "agent-dev", attach: true})
...
func TestRunAgentClosesSessionWhenPaneCreationFails(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	session := &fakeSession{name: "agent", newPaneErr: errors.New("pane failed")}
...
func TestRunAgentClosesSessionWhenLaunchFails(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
...
func TestRunAgentPropagatesAttachFailure(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	session := &fakeSession{
		name:      "agent",
	...
	exitCode := runAgent(NewRunnerConfig(
		...
		WithBuildLaunch(func(string, ...string) string { return "launch agent" }),
	), parsedArgs{sessionName: "agent", attach: true})
