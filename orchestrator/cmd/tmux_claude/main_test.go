...
func TestRunClaudeLaunchesWithoutAttach(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	pane := &fakePane{target: "claude:0.0"}
	session := &fakeSession{name: "claude", pane: pane}

	exitCode := runClaude(NewRunnerConfig(
		WithStdout(&stdout),
		WithStderr(&stderr),
	...
	if len(pane.sent) != 1 || pane.sent[0] != "launch claude" {
		t.Fatalf("unexpected sent launcher: %v", pane.sent)
	}
	if !strings.Contains(stdout.String(), `Launched Claude in tmux session "claude"`) {
...
func TestRunClaudeAttachesWhenRequested(t *testing.T) {
	stdin := strings.NewReader("interactive input")
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	pane := &fakePane{}
	session := &fakeSession{name: "claude-dev", pane: pane}
	...
	exitCode := runClaude(NewRunnerConfig(
		...
		WithBuildLaunch(func(string, ...string) string { return "launch claude" }),
	), parsedArgs{sessionName: "claude-dev", attach: true})
...
func TestRunClaudeClosesSessionWhenPaneCreationFails(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	session := &fakeSession{name: "claude", newPaneErr: errors.New("pane failed")}
...
func TestRunClaudeClosesSessionWhenLaunchFails(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
...
func TestRunClaudePropagatesAttachFailure(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	session := &fakeSession{
		name:      "claude",
	...
	exitCode := runClaude(NewRunnerConfig(
		...
		WithBuildLaunch(func(string, ...string) string { return "launch claude" }),
	), parsedArgs{sessionName: "claude", attach: true})
